...
window.checkActionsStatus = async function(showToastOnManual) {
  ...
  try {
    ...
    if (data.status !== "ok" && data.success !== true) {
      throw new Error(data.error || data.message || "取得に失敗しました");
    }
    ...
    const createdTime = latest.created_at || latest.updated_at || new Date().toISOString();
    const timeStr = new Date(createdTime).toLocaleTimeString();
    const runNum = latest.run_number || latest.id || "?";
    if (meta) meta.textContent = "開始時刻: " + timeStr + " (Run ID: #" + runNum + ")";
